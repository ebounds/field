...
6. **How is this held?** Name which of this reading you met directly and which you reconstructed or inferred. A stance drawn from a summarized opening, a compacted middle, or a few retrieved lines is not held the same way as one read in full. Say so as a phrase, not a percentage.
7. **What is actually available?** Use the synthesis procedure to note missing phases and populate the separate `field-coverage` metadata.
...
| How it is held | `grounding`; keep it consistent with `source_basis` and `phase_coverage`, and distinct from `definition`. |
...
Question 6 is the one most easily skipped, because an assistant's reading nearly always mixes what it met with what it filled in, and prose flattens that into hedging or silence. Answer it plainly. It is a fact about the expression, not a confession, and `grounding` gives it somewhere to go.

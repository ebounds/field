...
try {
  ...
  assert((await evaluate("document.querySelector('#audio-moment').textContent")).startsWith('Selected for listening: Moment 3'));
  ...
  const heardDescription=await evaluate("document.querySelector('#audio-description').textContent");
  await publish('a',6,'amber');await waitSequence(6);
  assert.equal(await evaluate("document.querySelector('#audio-moment').textContent"),'Hearing: Expression 5 · In view: Expression 6');
  assert.equal(await evaluate("document.querySelector('#audio-description').textContent"),heardDescription);
  assert.equal(await evaluate('window.audioAudit.starts'),1);
  await screenshot('companion-audio-moment');
  await click('#listen');
  assert.equal(await evaluate("document.querySelector('#audio-moment').textContent"),'Selected for listening: Expression 6');
  await click('#listen');await waitFor("document.querySelector('#listen').textContent==='Stop listening'");
  assert.equal(await evaluate("document.querySelector('#audio-moment').textContent"),'Hearing: Expression 6');
  assert.notEqual(await evaluate("document.querySelector('#audio-description').textContent"),heardDescription);
